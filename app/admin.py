...
def admin_add_class():
    if 'loggedIn' in session:
        if check_permissions() > 2:
            ...
            sql_data.execute("""SELECT * FROM pool;""")
            pool_list = sql_data.fetchall()
            return render_template('admin/add_class.html', instructor_list=instructor_list, class_list=class_list, pool_list=pool_list, time=start_time, date=complete_date_string)
        else:
            return redirect(url_for('index'))
    else:
        return redirect(url_for('login'))
...
@app.route('/admin_edit_class', methods=['POST'])
def admin_edit_class():
    if 'loggedIn' in session:
        if check_permissions() > 2:
            class_id = request.form.get('class_id')
            render_template('admin/edit_class.html')
        else:
            return redirect(url_for('index'))
    else:
        return redirect(url_for('login'))
