...
def member_list():
    ...
    if 1:
        ...
        if 1:
            sql_data = get_cursor()
            sql = """SELECT member.member_id,member.user_id,member.first_name,member.last_name,member.phone_number,
                    member.detailed_information,member.street_name,member.birth_date,member.health_information,
                    title.title,city.city,region.region 
                    FROM member 
                    LEFT JOIN title ON member.title_id = title.title_id
            ...
            return render_template("member_list.html", member_list_result=member_list_result)
        else:
            return url_for("/")
    else:
