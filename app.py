...
def index():
    ...
    print(session, 'loggedIn' in session)
...
def dashboard():
    if 'loggedIn' in session:
        if session['is_instructor'] == 1:
            ...
            return render_template('root/root_dashboard.html', permissions=check_permissions())
        else:
...
def user_list():
    if 'loggedIn' in session:
        if check_permissions() > 2:
            ...
            sql_data.close()
            return render_template("admin/user_list.html", member_list=member_list, instructor_list=instructor_list, permissions=check_permissions())
        else:
            return redirect(url_for('index'))
    else:
        return redirect(url_for('login'))
